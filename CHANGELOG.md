...
- **多寵物（子專案 B1）**：每個**啟用的頻道** + 「全部」各長出**一隻寵物視窗**，各自造型、各自反應符合的事件、各自未讀紅點；從「全部」向左堆疊定位、channel 寵物可拖（不自走）；「全部」維持完整行為（走動/即時卡片）。事件反應：「全部」反應所有訊息、命中的頻道寵物額外反應（多屬來源→多隻一起跳），嫌吵可用「全部」開關關掉那隻。**點寵物開通知中心會切到該頻道分頁**。reconcile 生命週期（啟用→生、停用/刪→收、`allEnabled`→「全部」），保證至少 1 隻寵物以防鎖死。寵物以 URL `?c=<channelId>` 辨識身分、per-pet 命令帶 channelId 路由（`window.ts` 單→多寵物重構）。新增純函式 `pet-layout.stackPosition`。channel 寵物的**即時卡片 / 拖曳位置記憶**為子專案 B2。
