...
在 desktop-notify 桌面寵物 app 中加入蕃茄鐘功能。功能以 **per-channel opt-in** 方式整合。蕃茄鐘是內建通知來源——**繞過 HTTP ingest pipeline 與 MessageStore**，由 `pomodoro-driver` 直接呼叫 `showInternalCard()` 顯示即時通知 card，不走 `autoDetectChannel`，不進通知中心，不污染 channel/source 管理系統。

本設計引入**訊息二分法**（domain 概念），並包含一個獨立 UX 修正：**清空按鈕語意對齊**（見末節）。
...
| | **持久訊息（persistent message）** | **即時通知（transient notice）** |
|---|---|---|
| 來源 | 外部 ingest（hooks / `POST /notify`） | 內建功能（蕃茄鐘為第一個 use case） |
| MessageStore | 進 | **不進** |
| 通知中心 | 可回看 | **不出現** |
| 已讀 / badge | 有 | **無** |
| Card 行為 | persistent，手動 dismiss | **自動消失（dismissMs）** |
| CardView 標記 | `transient` 欄位 `undefined` | `transient: { dismissMs }` |

行為一致性原則：一則訊息屬於哪類，由產生它的來源決定，全鏈路行為跟著類別走，不混用。

---

## Scope

### 功能 1：Transient card（即時通知 card）

`CardView`（`src/core/card-view.ts`）加選用欄位：

```typescript
transient?: { dismissMs: number }
```

- `undefined` → 持久訊息 card（現有行為，所有現有通知不變）
- `transient: { dismissMs: 5000 }` → card renderer `setTimeout` 倒數後自動關閉

**不動 `AppEvent.ttlMs`**（其預設 5000，語意是訊息存活時間，與 transient 概念無關）。零回歸風險。

未來 transient 類若有更多行為差異（不同視覺樣式等），統一掛在 `transient` 欄位下，語意集中。

實作注意（card renderer，`src/renderer/card.ts`）：
- 每次 render 新 view 時清掉舊 timer，捕捉 render 當下的 card `id`，避免舊 timer 關掉新 card
- Transient card 不在 MessageStore → 無已讀概念，timeout 直接走 card 關閉路徑（不需 markRead）

### 功能 2：蕃茄鐘

使用者啟用後，在選定 pet 上 hover 顯示控制列（倒數 + 按鈕），phase 結束時以**即時通知 card** 提醒，5 秒自動消失。依二分法：不存 MessageStore、不進通知中心、無未讀 badge。
...
src/core/card-view.ts          ← CardView 加 transient 欄位
...
用戶按 ▶
  ...
  → driver 呼叫 showInternalCard({ title, body, transient: { dismissMs: 5000 } })
...
- Effect 處理：
  - `notify-work-end` → `showInternalCard({ title: '🍅 休息一下！', body: '工作時間結束，好好休息。', transient: { dismissMs: 5000 } })`
  - `notify-break-end` → `showInternalCard({ title: '⏰ 繼續工作！', body: '休息結束，下一個蕃茄開始。', transient: { dismissMs: 5000 } })`
...
- 組裝 `CardView`（含 `transient`），**不寫入 MessageStore**
...
| **工作結束** | elapsedMs ≥ workMs | 即時通知 card：「🍅 休息一下！」5s 自動消失 |
| 休息中 | 自動接續 | hover bar 藍色倒數 |
| **休息結束** | elapsedMs ≥ breakMs | 即時通知 card：「⏰ 繼續工作！」5s 自動消失 |
