...
export interface Channel {
  id: string          // main 產生的 opaque id；路由/去重看 match 不看 id
  name: string        // 顯示名
...
// 回所有「enabled 且 matchesSource」的 channel id。注意：'all' 不在此清單（隱含含全部，由 UI/呼叫端處理）。

filterByChannel(messages, channelId, channels): Message[]
// channelId==='all' → 全部；否則 → matchesSource 命中該 channel 的訊息（忽略 enabled，供管理/預覽；分頁端只對 enabled 呼叫）
...
// 驗證陣列：id/name/string、skin 字串、enabled boolean、match 至少一有效欄；壞的丟棄
```
...
收到事件（`onEvent`）時，對 `event.source` 判定：
- 若 **(a)** `matchingChannels(source, channels)` 為空（沒有任何**啟用** group 命中）**且 (b)** 不存在 match 完全等於 `{kind,name}` 的既有 channel（含停用）→ **自動新增** 一個 channel：`{ id: <main 產生>, name: source.name || source.kind, skin: prefs.skin, enabled: false, match: { kind: source.kind, name: source.name }（name 缺則只 kind） }`，`savePrefs` + 推 `channels-updated`。
- 廣域啟用 channel（如 `{kind:'claude-code'}`）底下不會再長一堆停用子項（因 (a) 已命中）。
...
- 窄版 preload `src/preload/channels.ts` 暴露 `channelsBridge`：`getChannels` / `upsertChannel` / `deleteChannel` / `onChannelsUpdated` / `getSkins`（直接走 ipcRenderer，不依賴 preload-helpers，避免共用 chunk —— 比照 card preload）。
