...
**匯入規則（Codex Q5）：** renderer（web tsconfig：center.ts / channels.tsx）只 import `src/core/*`，**不得 import `src/ipc/contract`**（contract type-import `../main/prefs` 等 Node 模組，會把 Node type graph 拉進 web 編譯）。channel 型別放 `core/channel.ts` 即為此。

**prefs 單一寫入慣例（Codex Q1）：** 所有 prefs.json 寫入一律走 `updatePrefs(userDataDir, partial)`（讀最新→合併→寫），main / window.ts 都只更新自己負責的欄位，避免互相覆蓋 channels。
...
## Task 2: prefs 加 `channels` + `updatePrefs` 合併寫入 + 修 window.ts

> **Codex Q1/Q2**：prefs.json 有**兩個寫入者**（`window.ts` 寫 autoWalk/dnd/walk/skin；`index.ts` 將寫 channels）。各自寫整份快照會互相覆蓋。解法：加 `updatePrefs(userDataDir, partial)`（讀最新→合併→寫），兩邊都只改自己欄位。並修 `window.ts` 的 Prefs literal（channels 變必填）。

**Files:**
- Modify: `src/main/prefs.ts`、`src/main/window.ts`

- [ ] **Step 1: prefs.ts — channels 欄位 + sanitize + updatePrefs**

`src/main/prefs.ts`：頂部 import 加 `import { sanitizeChannels, type Channel } from '../core/channel'`。

`Prefs` 加 `channels: Channel[]`；`DEFAULTS` 加 `channels: []`；正常解析 return 加 `channels: sanitizeChannels(parsed.channels)`；兩個 fallback return 加 `channels: []`。

檔尾新增合併寫入 helper（單一寫入入口、避免覆蓋）：
...
- [ ] **Step 2: window.ts — literal 補 channels + 寫入改用 updatePrefs**

`src/main/window.ts`：
- import 改 `import { loadPrefs, savePrefs, updatePrefs, type Prefs } from './prefs'`（保留 savePrefs 給 window-state 無關處；實際改用 updatePrefs）。
- 模組層 literal 補 `channels: []`：
  ```ts
  let prefs: Prefs = {
    autoWalk: true,
  ...
  ```
- 把所有 `savePrefs(app.getPath('userData'), prefs)` 改為**只更新自己欄位的合併寫入**，並用回傳更新本地快照。例如自動走動：
  ```ts
  prefs = updatePrefs(app.getPath('userData'), { autoWalk: prefs.autoWalk })
  ```
  其餘 dnd（applyDnd）、walk（set-walk-bounds）、skin（select-skin）同理改成 `updatePrefs(userData, { dnd })` / `{ walk }` / `{ skin }`。**不要再寫整份 prefs**（否則蓋掉 channels）。
...
```bash
git add src/main/prefs.ts src/main/window.ts
git commit -m "feat(prefs): channels 欄位 + updatePrefs 合併寫入；window.ts 改走 updatePrefs 避免覆蓋 channels"
```
...
```
（`loadPrefs`/`savePrefs` 已 import；再加 `updatePrefs`：`import { loadPrefs, savePrefs, updatePrefs } from './prefs'`。）
...
（auto 建的 match 一律含 kind、有 name 才加 name —— 與 `needsAutoChannel` 的去重比對一致。）
...
- [ ] **Step 3: center.ts — 取得 channels（追加，勿覆蓋既有載入）**

**不要改動既有的 `getMessages().then(...)` / `consumePendingDetail()` / `onOpenDetail` 區塊**（Spec ⑧ 的詳情流程要保留）。在那些之後**追加**：
...
- [ ] **Step 2: build（驗證 channels 入口 + preact 產出 + preload 無共用 chunk）**

Run: `npm run build`
Expected: 成功；`out/renderer/channels.html`、`out/preload/channels.cjs` 存在。
再驗 preload 各入口自含（Codex Q4，先前踩過 chunk 載入失敗）：
Run: `ls out/preload/ && grep -RIl "chunks/" out/preload/*.cjs || echo "no chunk deps OK"`
Expected: `out/preload/` 無 `chunks/` 目錄、各 .cjs 不 require `./chunks/...`。
...
- [ ] **Step 3b: dev 模式 CSP 檢查（Codex Q3）**

Run: `npm run dev`，右鍵「頻道…」開 channels 視窗。
Expected: DevTools console **無 CSP 違規**（Preact/signals 無 eval）。若 `@preact/preset-vite` 的 dev HMR（prefresh）觸發 `eval`/`new Function` 而報 CSP 錯：在 channels.html 的 CSP **僅 dev** 加 `'unsafe-eval'`（用 `process.env.ELECTRON_RENDERER_URL` 條件，或停用 prefresh：`preact({ prefreshEnabled: false })`）。production build 不受影響。
