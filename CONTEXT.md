...
`cardReduce` 的 adapter：`src/main/card-manager.ts` 的 `initCardManager(deps) → { show, closeFor }`（dismiss 由 card-clicked IPC 在 module 內處理）。持有 per-channel BrowserWindow 與狀態、執行指令副作用（顯示/隱藏/定位/通知已讀）、自訂閱 bus 拖動事件做卡片同步、自註冊 card domain 的 IPC handler（show-card／hide-card／card-clicked／card-more）。
