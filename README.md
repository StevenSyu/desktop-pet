...
- **色彩編碼通知卡片**（左色條＋同色狀態標籤、無 emoji），暖白卡面、SF Rounded 圓體字。**持久顯示直到點關閉或被新訊息替換**。卡片為**獨立浮動小視窗**，浮在寵物上方（上方不足自動翻下方）、跟著寵物拖動移動；寵物視窗本身縮成 sprite 大小，可一路拖到主螢幕最上方貼到選單列。內文顯示精簡首段，**點卡片本體開通知中心詳情看全文、右上角 ✕ 關閉**。同一訊息在多個頻道彈出的多張卡片，**關一張其餘連帶關閉**（依 event id）。
- **通知中心**：所有訊息進歷史佇列（容量 50）、已讀/未讀、寵物未讀數徽章（**點一下徽章直接開通知中心**）、狀態 chips 篩選、時間分組（剛剛／今天稍早／更早）、**單則詳情面板**（安全 Markdown 渲染：粗體／程式碼／清單／表格，完整 metadata；Esc 兩段式返回）、全部已讀／清空、× 或 Esc 關閉。中心**可拖曳並記住最後位置**；首次開在**寵物所在螢幕、靠寵物那側**。
- **通知頻道 / 群組**：把通知依來源（專案／類別）分成**可重疊的頻道**；右鍵「寵物設定…」開管理視窗，**左右兩欄**把「已知來源」拖／點進頻道（**可跨專案合併到同一頻道**）。來源帶 **agent kind 色標**（claude／codex／curl），池子依 kind 分組、每組最上方有**「全部來源」整類項**（虛線框）——拖進頻道即涵蓋該 agent 的所有來源（含未來新專案），並自動吸收同 kind 精確成員。**新來源自動建啟用頻道並跳出一隻專屬寵物演出首訊息**。啟用的頻道在通知中心有自己的分頁。頂部「全部」頻道含開關。
- **多寵物**：每個啟用的頻道 +「全部」各一隻寵物，各自造型/反應/未讀紅點，從「全部」向左排、可拖；channel 寵物不自走、「全部」完整（走動/卡片）。事件在「全部」+ 命中頻道寵物上反應（嫌吵可關「全部」那隻）；點寵物開通知中心會切到該頻道分頁。每隻寵物有**各自的即時卡片**與**各自記憶的拖曳位置**。
...
- **renderer**：`index.html`（寵物＋徽章）／`card.html`（即時卡片獨立視窗）／`center.html`（通知中心面板）／`channels.html`（頻道管理，Preact）。
- **preload**（`src/preload/`）：`contextBridge` 暴露 `window.petBridge`（主視窗）、窄版 `window.cardBridge`（卡片視窗）、`window.channelsBridge`（頻道管理）API。
...
**已完成**：核心庫、Electron 外殼（透明置頂／點擊穿透／all-spaces）、3 隻可換造型＋造型記憶、色彩編碼即時卡片、Hook Kit（含 Stop 抓 transcript 最後文字、cwd 過濾 retry 防 race）、通知中心（歷史／未讀徽章／篩選／分組／展開）、視窗行為（拖動記憶／floating 層級／多螢幕重吸附）、動畫與效能（idle 走動／撞牆反向／CSS-only sprite／不可見暫停／進階設定）、寵物互動（hover/click/dblclick/drag 方向 sprite 反應）、勿擾模式、造型掃描與選擇 UI（pet:// 動態載圖）、即時卡片獨立視窗＋寵物縮窗（拖到最上方貼選單列、走動暫停/中斷）、卡片「更多」→ 通知中心單則詳情面板（安全 Markdown 渲染含表格、完整 metadata、中心跟隨寵物螢幕）、通知頻道/群組（多寵物子專案 A：來源分群、左右兩欄拖拽、跨專案合併、通知中心分頁）、多寵物（子專案 B1：每啟用頻道+全部各一隻寵物、各自造型/反應/紅點、向左堆疊、點寵物開對應分頁；子專案 B2：每隻寵物各自即時卡片、每頻道拖曳位置跨重啟記憶）、寵物 channel 名稱標籤（腳邊 pill、三態隱藏/hover/常態）、造型視窗認 channel（右鍵 per-pet／頻道頁外開造型頁／「全部」納入頻道頁造型設定）、可調整寵物大小（hover 出現右下把手拖曳、左上固定、每隻各自、跨重啟記住）、通知中心按 session 篩選 chips（同專案多 session 可分開看，≥2 session 才顯示）、通知中心可拖曳＋記住最後位置、同一訊息多張卡片連帶關閉（依 event id）、來源 agent kind 色標、kind 整類來源（「全部來源」池選項、依 kind 分組排序當 group header、加入時吸收同 kind 精確成員、啟動補齊缺的整類項）、新來源自動建啟用頻道跳專屬寵物（首訊息等視窗載入完成再推，不丟失）。
